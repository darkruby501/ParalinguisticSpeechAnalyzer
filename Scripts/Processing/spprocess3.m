function [Sp] = spprocess3(filename,reSFS,SFSreuse,reCalc,Sp)
...
for i = 1:length(Sp.Utter)
    [HRT,ratio,diff] = hrtdetector2(Sp.Utter(i).FIntrp,Sp.fs0,HRT_syll,HRT_phrase,HRTthresh,Sp.PitchStat.Fmean);
...
Sp.HRTstat.HRTthresh = HRTthresh;
...
for i = 1:length(Sp.Utter)
    [FI,ratio,diff] = fidetector(Sp.Utter(i).FIntrp,Sp.fs0,HRT_syll,HRT_phrase,FIthresh,Sp.PitchStat.Fmean);
    Sp.Utter(i).FIratio = ratio;
    Sp.Utter(i).FI = FI;
    Sp.Utter(i).FIdiff = diff;
end

Sp.FImat = [vertcat(Sp.Utter(:).FIratio) vertcat(Sp.Utter(:).FI) vertcat(Sp.Utter(:).FIdiff)];
Sp.FIstat = fistat(Sp.FImat);
Sp.FIstat.FIthresh = FIthresh;
