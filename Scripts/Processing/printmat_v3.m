function [] = printmat_v3(a,name,rlab,clab,precision)
  ...
  for i=1:nrows,
    ...
    for j=0:n,
      ...
      elseif (element>=1.e6)|(element<=-1.e5)|(abs(element)<.0001)
        s=[s,sprintf([' %12.' num2str(precision) 'e'],element)]; %s=[s,sprintf(' %12.5e',element)];
      else
        s=[s,sprintf([' %12.' num2str(precision) 'f'],element)];
      end
    end
    disp(s)
  end % for
